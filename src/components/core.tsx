...
export const Pre: React.FC<{}> = ({ children }) => {
    ...
    return (<>
        <button className={"block p-4 text-sm btn " + (pressed ? "bg-green-400" : "bg-gray-400")} onClick={async () => {
        ...
        <pre ref={preEl} className="block bg-gray-300 p-4 text-sm overflow-scroll">{children}</pre>
    </>)
}
