...
const SplitOnce = (match: string, text: string): { head: string, tail: string } => {
...
type RecurseResult = { newMatches: string[], newText: string, newAccumulated: ParsedElement[] };
const ParseRecursive =
    (oldMatches: string[], oldText: string, oldAccumulated: ParsedElement[]): RecurseResult => {
...
const CaptureAndDecorate =
    <T>(regex: RegExp, plain: DecoratePlain<T>, capture: DecorateCapture<T>, text: string): T[] => {
        const matches = text.match(regex);
        if (matches !== null) {
            const {newAccumulated, newText} = ParseRecursive(matches,text,[]);
            const finalElements: ParsedElement[] = [...newAccumulated,{type: 'plain',text: newText}];
            return finalElements.map(el => {
                const fn = el.type === 'decorated'
                    ? capture
                    : plain;
                return fn(el.text);
            });
